/**
 * accepter_pap-scripts.bundle.js
 * Page "Décision du DG" : liste les dossiers ayant reçu un avis favorable
 * de la DFC (passer_achat_et_paiement.idStatut = 3), avec 3 actions par ligne :
 ...
 *   - Accepter : fait passer idStatut de 3 à 4.
 *   - Rejeter : fait passer idStatut de 3 à 5, et réinitialise montant_total
            ...
            { targets: 5, render: d => d !== null ? '<span class="dga-cell-amount">' + dga_formatMontant(d) + '</span>' : '<span style="color:#d1d5db;">—</span>' },
            {
                ...
                render: (d) => `
                    <div style="display:flex;gap:.4rem;">
                        ...
                        <button type="button" class="dga-btn-avis" onclick="dga_confirmerAccepter('${d}')">
                            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
...
function dga_afficherDocuments(dossier, res) {
    ...
    if (estAchat) {
        titre.textContent = 'Facture définitive';
        ...
        conteneur.innerHTML = `
            <div class="dga-doc-item">
                ...
                <div class="dga-doc-info">
                    <div class="dga-doc-nom">Facture définitive</div>
...
function dga_confirmerAccepter(token) {
    Swal.fire({
        ...
        text: 'Confirmez-vous l\'acceptation de ce dossier ?',
        icon: 'question',
        ...
        dga_envoyerDecision(token, 3, 'Acceptation du dossier…', 'Dossier accepté avec succès.');
    });
}
...
function dga_confirmerRejeter(token) {
    Swal.fire({
        ...
        html: `
            <p style="margin-bottom:.5rem;text-align:left;">Le montant total et le prix réel/montant déjà saisis pour chaque ligne seront réinitialisés.</p>
            <textarea id="dgaMotifRejet" class="swal2-textarea" placeholder="Motif du rejet (obligatoire)" style="margin-top:.5rem;"></textarea>
        `,
        icon: 'warning',
        showCancelButton: true,
        ...
        focusConfirm: false,
        preConfirm: () => {
            const motif = document.getElementById('dgaMotifRejet').value.trim();
            if (!motif) {
                Swal.showValidationMessage('Le motif de rejet est obligatoire.');
                return false;
            }
            return motif;
        },
        ...
        if (!result.isConfirmed) return;
        dga_envoyerDecision(token, 4, 'Rejet du dossier…', 'Dossier rejeté avec succès.', { motifRejet: result.value });
    });
}

function dga_envoyerDecision(token, option, messageChargement, messageSucces, donneesSupplementaires) {
    dga_showLoader(messageChargement);

    $.ajax({
        url: DG_CONTROLLER_URL,
        method: 'POST',
        data: Object.assign({ option: option, token: token }, donneesSupplementaires || {}),
        dataType: 'json'
    }).done(function (res) {
        dga_hideLoader();
        if (res.status === 'success') {
            Swal.fire({ title: 'Succès', text: res.message || messageSucces, icon: 'success', confirmButtonColor: '#1a7a5e' });
            chargerDossiers();
        } else {
