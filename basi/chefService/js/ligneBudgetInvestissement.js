// ═══════════════════════════════════════════════════════════════════════
// LIGNES DE BUDGET — Investissement uniquement
// POST JSON → chef_service_basi_controller
// ═══════════════════════════════════════════════════════════════════════

const LB_API = '/personnel/chef_service_basi_controller';
const lb_api = {
    budgetInfo:   `${LB_API}?option=18`, // POST { budgetId }
    lines:        `${LB_API}?option=19`, // POST { budgetId, id_type_budget_investissement? }
    lineById:     `${LB_API}?option=20`, // POST { lineId }
    createLine:   `${LB_API}?option=21`, // POST { budgetId, ... }
    updateLine:   `${LB_API}?option=22`, // POST { lineId, ... }
    deleteLine:   `${LB_API}?option=23`, // POST { lineId }
    rubriques:    `${LB_API}?option=24`, // POST {}
    sousRub:      `${LB_API}?option=25`, // POST { categorie_id }
    natures:      `${LB_API}?option=26`, // POST {}
    services:     `${LB_API}?option=27`, // POST {}
...
let lb_budgetType     = null; // type_budget_id (toujours 2 = investissement)
let lb_budgetCeil     = 0;
...
// et si le budget est validé (→ toutes les lignes actives sont verrouillées).
...
function lb_applyNatureLayout(categorie) {
    ...
    document.getElementById('lb-autre-row')?.classList.toggle('lb-hidden', isProduit || !hasNature);

    // Réinitialiser les erreurs à chaque changement de type
    lb_clearErrors();
}
...
async function lb_loadServices() {
    const r = await lb_post(lb_api.services, {});
    if (!r.ok) return;
    const d = await lb_checkResp(r); if (!d) return;
    const sel = document.getElementById('lb-service');
    if (!sel) return;
    sel.innerHTML = '<option value="">Service (optionnel)</option>';
    (d.services||[]).forEach(s => sel.insertAdjacentHTML('beforeend',
        `<option value="${s.id}">${s.nom_services}</option>`));
}

async function lb_loadNatures(selectId='lb-nature', filterId=null) {
    const r = await lb_post(lb_api.natures, {});
    if (!r.ok) return;
    const d = await lb_checkResp(r); if (!d) return;
    const sel = document.getElementById(selectId);
    if (sel) {
        sel.innerHTML = '<option value="">Sélectionner le type d\'investissement *</option>';
        (d.data||[]).forEach(n => sel.insertAdjacentHTML('beforeend',
            `<option value="${n.id}" data-categorie="${n.categorie}">${n.nom}</option>`));
    }
    if (filterId) {
        const flt = document.getElementById(filterId);
        if (flt) {
            flt.innerHTML = '<option value="">Tous les types</option>';
            (d.data||[]).forEach(n => flt.insertAdjacentHTML('beforeend',
...
function lb_updateCounters(donnees) {
    // Lire montant_total directement depuis la BDD (calculé côté serveur)
    lb_totalAmount = donnees.reduce((sum, l) => {
        return sum + (parseFloat(l.montant_total) || 0);
    }, 0);
    ...
    // Badge verrouillées dans l'en-tête tableau
    ...
    // Plafond + consommé + pourcentage
    // Toujours afficher le consommé, même si pas de plafond
    ...
    if (lb_budgetCeil > 0) {
        ...
        // Pas de plafond défini : barre à 0, pourcentage N/A
        const fill = el('lb-progress');
...
function lb_updateValidateBtn(donnees) {
    ...
    if (!btn) return;
    // Visible uniquement si budget modifiable et au moins 1 ligne
...
function lb_buildColumns() {
    ...
    return [
        { data:'nature_nom',            title:"Type d'investissement", defaultContent:'N/A' },
        { data:'categorie_nom',         title:'Rubrique',              defaultContent:'—' },
        { data:'sous_categorie_nom',    title:'Sous-rubrique',         defaultContent:'—' },
        ...
        { data:null, title:'Qté / Nbre', render: d => {
                const v = parseFloat(d.quantite);
                return (!v || v === 0) ? '<span style="color:#d1d5db;">—</span>' : v.toLocaleString('fr-FR');
            }},
        { data:'prix_unitaire', title:'Montant / P.U. (FCFA)', render: d => {
...
async function lb_initTable() {
    ...
    const body = { budgetId: lb_budgetId };
    const natFlt = document.getElementById('lb-filter-nature');
    if (natFlt?.value) body.id_type_budget_investissement = natFlt.value;
...
// ─── Infos budget ─────────────────────────────────────────────────────
// Chargement en parallèle : budget + services + natures + lignes en une seule passe
async function lb_loadBudgetInfo() {
    lb_showLoader('Chargement…');
    try {
        // Lancer toutes les requêtes en même temps
        const [rBudget, rServices, rNatures] = await Promise.all([
            lb_post(lb_api.budgetInfo, {budgetId: lb_budgetId}),
            lb_post(lb_api.services,   {}),
            lb_post(lb_api.natures,    {}),
        ]);

        // ── Budget ──────────────────────────────────────────────────
        if (!rBudget.ok) { lb_hideLoader(); Swal.fire('Erreur','Budget introuvable.','error'); return; }
        ...
        // ── Services ────────────────────────────────────────────────
        if (rServices.ok) {
            const dSvc = await lb_checkResp(rServices);
            if (dSvc) {
                const sel = document.getElementById('lb-service');
                if (sel) {
                    sel.innerHTML = '<option value="">Service (optionnel)</option>';
                    (dSvc.services||[]).forEach(s => sel.insertAdjacentHTML('beforeend',
                        `<option value="${s.id}">${s.nom_services}</option>`));
                }
            }
        }

        // ── Natures (modale + filtre tableau) ───────────────────────
        if (rNatures.ok) {
            const dNat = await lb_checkResp(rNatures);
            if (dNat) {
                const sel = document.getElementById('lb-nature');
                const flt = document.getElementById('lb-filter-nature');
                if (sel) {
                    sel.innerHTML = '<option value="">Sélectionner le type d\'investissement *</option>';
                    (dNat.data||[]).forEach(n => sel.insertAdjacentHTML('beforeend',
                        `<option value="${n.id}" data-categorie="${n.categorie}">${n.nom}</option>`));
                }
                if (flt) {
                    flt.innerHTML = '<option value="">Tous les types d\'investissement</option>';
                    (dNat.data||[]).forEach(n => flt.insertAdjacentHTML('beforeend',
                        `<option value="${n.id}">${n.nom}</option>`));
                }
            }
        }
...
async function lb_submitForm(e) {
    ...
    // Période : toujours "Début-Fin" même si identiques (ex: "Mars-Mars")
    let periode = '';
    if (debut && fin) periode = `${debut}-${fin}`;
    else if (debut)   periode = debut;

    const natSel  = document.getElementById('lb-nature');
    const opt     = natSel.options[natSel.selectedIndex];
    const natureId  = natSel.value;
    const categorie = opt ? opt.getAttribute('data-categorie') : '';

    // ── Validation inline côté client ──────────────────────────────
    let hasError = false;
    ...
    const payload = {
        ...
        id_type_budget_investissement: natureId,
        designation,
        description:           document.getElementById('lb-description').value.trim(),
        periode_d_utilisation: periode,
        service_id:            document.getElementById('lb-service').value || null,
    };
    ...
    if (categorie === 'Produit') {
        ...
        payload.nombre        = parseFloat(document.getElementById('lb-nombre').value)||0;
        payload.montant_total = parseFloat(document.getElementById('lb-montant').value)||0;
        if (payload.nombre <= 0) {
            lb_showError('lb-nombre','err-nombre','Le nombre est requis et doit être supérieur à 0.');
            hasError = true;
        }
        if (payload.montant_total <= 0) {
            lb_showError('lb-montant','err-montant','Le montant total doit être supérieur à 0.');
            hasError = true;
        }
    }
    ...
    if (categorie) {
        ...
        if (!debut) {
            ...
            // Erreur sur le select fin (pas début)
            ...
            // Fin ne peut pas être antérieure à début
            if (idxFin < idxDebut) {
                const selFin = document.getElementById('lb-periode-fin');
                if (selFin) selFin.classList.add('lb-error');
    ...
    // Arrêter si erreurs — le formulaire reste ouvert avec les messages
    if (hasError) return;
    ...
    lb_closeModal();
    // Attendre que le swal de succès se ferme avant de relancer le loader du tableau
...
async function lb_editLine(lineId) {
    ...
    document.getElementById('lb-designation').value = l.designation||'';
    ...
    // 1) Charger les natures et services (await garantit qu'ils sont prêts)
    await lb_loadNatures('lb-nature');
    await lb_loadServices();

    // 2) Sélectionner le service après chargement
    document.getElementById('lb-service').value = l.service_id||'';

    // 3) Sélectionner la nature et déduire la catégorie
    const natSel = document.getElementById('lb-nature');
    natSel.value = l.id_type_budget_investissement||'';
    const opt = natSel.selectedIndex >= 0 ? natSel.options[natSel.selectedIndex] : null;
    const categorie = opt ? (opt.getAttribute('data-categorie')||'') : '';

    // 4) Appliquer le layout selon la catégorie
    lb_applyNatureLayout(categorie || null);

    if (categorie === 'Produit') {
        // a) Charger les rubriques puis sélectionner la bonne
        ...
        // Type Autre : nombre = quantite, montant_total = montant_total
        document.getElementById('lb-nombre').value  = l.quantite||'';
        document.getElementById('lb-montant').value = l.montant_total||'';
    }
...
async function lb_validerBudget() {
    ...
    const d = await lb_checkResp(r); if (!d) return;
    if (!d.success) { Swal.fire('Erreur',d.message,'error'); return; }
    // IMPORTANT : le statut est mis à jour AVANT lb_initTable()
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!lb_budgetId) { Swal.fire('Erreur','Token budget introuvable.','error'); return; }

    // Navigation retour
    document.getElementById('lb-back-btn')?.addEventListener('click', ()=>history.back());
    ...
    // Nature change → adapter le layout
    document.getElementById('lb-nature')?.addEventListener('change', function() {
        const opt = this.options[this.selectedIndex];
        const cat = opt ? opt.getAttribute('data-categorie') : null;
        ...
        if (cat === 'Produit') {
            ...
            // pré-remplir la désignation avec le nom du type si elle est vide
            const desig = document.getElementById('lb-designation');
            if (desig && !desig.value) desig.value = opt.text.trim();
        }
    });
