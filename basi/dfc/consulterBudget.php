...
// ─── Récupérer et valider l'ID (brut ou déjà chiffré passé en token) ─────────
function tokenencrypt($data): string {
    $key = hash('sha256', 'U@hbENTDRI@TCRI@T2022');
    $iv  = substr(hash('sha256', 'www.ent.uahb.sn'), 0, 16);
    return base64_encode(openssl_encrypt($data, 'AES-256-CBC', $key, 0, $iv));
}
function tokendecrypt($data): string|false {
    $key = hash('sha256', 'U@hbENTDRI@TCRI@T2022');
    $iv  = substr(hash('sha256', 'www.ent.uahb.sn'), 0, 16);
    return openssl_decrypt(base64_decode($data), 'AES-256-CBC', $key, 0, $iv);
}

// Accepte ?budgetId=N (brut, on chiffre) OU ?token=XXX (déjà chiffré)
$budgetId  = 0;
$budgetToken = '';
if (!empty($_GET['budgetId'])) {
    // Token déjà chiffré — on déchiffre pour valider l'ID
    $budgetId    = (int)tokendecrypt($_GET['budgetId']);
    $budgetToken = $_GET['budgetId'];
} elseif (!empty($_GET['budgetId'])) {
    $budgetId    = (int)$_GET['budgetId'];
    $budgetToken = tokenencrypt((string)$budgetId);
}
if (!$budgetId) { header('Location: /personnel/dfc-liste_budget'); exit; }
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <title>Consultation Budget — DFC</title>
    ...
    <div class="page d-flex flex-row flex-column-fluid">
        ...
        <div id="kt_aside" class="aside aside-light aside-hoverable" data-kt-drawer="true"
            ...
            <div class="aside-logo flex-column-auto text-center" id="kt_aside_logo">
                <a href="/personnel/dfc-accueil" style="margin-left:65px;" id="lien_logo1">
            ...
            <div id="kt_header" class="header align-items-stretch">
                <div class="container-fluid d-flex align-items-stretch justify-content-between">
                    ...
                    <div class="d-flex align-items-center flex-grow-1 flex-lg-grow-0">
                        <a href="/personnel/dfc-accueil" class="d-lg-none" id="lien_logo2">
                    ...
                    <div id="kt_content_container" class="container-xxl">
                        ...
                        <div class="cb-hero">
                            <div class="cb-hero-left">
                                <button id="cb-back-btn" class="cb-back">
                                ...
                                <div class="cb-hero-title">
                                    <h1>
                                        ...
                                        <span id="cb-titre">Chargement…</span>
                                    </h1>
                                    <p id="cb-sous-titre"></p>
                                </div>
                            </div>
                            <div class="cb-hero-right">
                                <span id="cb-type-chip" class="cb-type-chip">—</span>
                                <span id="cb-statut"></span>
                                <div id="cb-actions-wrap" style="display:flex;gap:.5rem;flex-wrap:wrap;"></div>
                            </div>
                        </div>
                        ...
                        <div style="padding:1.5rem 0">
                            ...
                            <!-- Message erreur -->
                            <div id="cb-error" class="hidden"></div>

                            <!-- ── KPI CARDS ── -->
                            <div class="cb-kpi-grid">
                                <div class="cb-kpi cb-kpi-1">
                                    <div class="cb-kpi__lbl">Plafond</div>
                                    <div class="cb-kpi__val" id="cb-kpi-plafond">—</div>
                                    <div class="cb-kpi__ico" style="background:#d1fae5;color:#1a7a5e">
                                ...
                                <div class="cb-kpi cb-kpi-2">
                                    <div class="cb-kpi__lbl">Consommé</div>
                                    <div class="cb-kpi__val" id="cb-kpi-consomme">—</div>
                                    <div class="cb-kpi__ico" style="background:#dbeafe;color:#2563eb">
                                ...
                                <div class="cb-kpi cb-kpi-3">
                                    <div class="cb-kpi__lbl">Reste dispo</div>
                                    <div class="cb-kpi__val" id="cb-kpi-reste">—</div>
                                    <div class="cb-kpi__ico" style="background:#d1fae5;color:#059669">
                                        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg>
                                    </div>
                                </div>
                                <div class="cb-kpi cb-kpi-4">
                                    <div class="cb-kpi__lbl">Lignes</div>
                                    <div class="cb-kpi__val" id="cb-kpi-lignes">—</div>
                                    <div class="cb-kpi__ico" style="background:#fef3c7;color:#d97706">
                                ...
                                <div class="cb-kpi cb-kpi-5">
                                    <div class="cb-kpi__lbl">Verrouillées</div>
                                    <div class="cb-kpi__val" id="cb-kpi-verr">—</div>
                                    <div class="cb-kpi__ico" style="background:#ede9fe;color:#7c3aed">
                                        <svg width="15" height="15" viewBox="0 0 24 24" fill="#7c3aed"><path d="M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zm-6 9c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2zm3.1-9H8.9V6c0-1.71 1.39-3.1 3.1-3.1 1.71 0 3.1 1.39 3.1 3.1v2z"/></svg>
                                    </div>
                                </div>
                                <div class="cb-kpi cb-kpi-6">
                                    <div class="cb-kpi__lbl">Année</div>
                                    <div class="cb-kpi__val" id="cb-kpi-annee">—</div>
                                    <div class="cb-kpi__ico" style="background:#eef2ff;color:#6366f1">
                            ...
                            <!-- ── PROGRESSION PLAFOND ── -->
                            <div class="cb-prog-card">
                                <div class="cb-prog-head">
                                    <span class="cb-prog-head-title">
                                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#1a7a5e" stroke-width="2.5"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
                                        Consommation du plafond
                                    </span>
                                    <span class="cb-prog-pct" id="cb-prog-pct">0%</span>
                                </div>
                                <div class="cb-prog-track">
                                    <div id="cb-prog-bar" class="cb-prog-fill ok" style="width:0%"></div>
                                </div>
                                <div id="cb-alert-depass" class="cb-alert-depass hidden">
                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="#991b1b"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>
                                    Le total des lignes dépasse le plafond du budget !
                                </div>
                            </div>
                            ...
                            <div class="cb-card">
                                <div class="cb-card-head">
                                    <span class="cb-card-title">
                                        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#1a7a5e" stroke-width="2.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                                        Informations du budget
                                    </span>
                                </div>
                                <div class="cb-info-grid" id="cb-info-grid">
                                    <!-- peuplé par JS cb_renderInfo() -->
                                </div>
                            </div>
                            ...
                            <div class="cb-card">
                                <div class="cb-tabs">
                                    <button class="cb-tab-btn active" data-tab="lignes">
                                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/></svg>
                                        Lignes budgétaires
                                    </button>                                </div>

                                <!-- Panel Lignes -->
                                <div id="cb-panel-lignes" class="cb-tab-panel active">
                                    <div class="cb-dt-wrap">
                                        <table id="cb-table-lignes" class="display" style="width:100%">
                                            <thead><tr></tr></thead>
                                            <tbody></tbody>
                                        </table>
                                    </div>
                                    <div class="cb-table-foot">
                                        ...
                                        <strong id="cb-table-total">0&nbsp;FCFA</strong>
                                    </div>
                                </div>
                            </div><!-- /.cb-card onglets -->
...
<!-- Token chiffré transmis au JS -->
<script>
    // Le token est généré côté PHP (tokenencrypt) et injecté ici.
    // Le JS l'envoie tel quel dans les requêtes POST → le PHP le déchiffre (tokendecrypt).
    window.CB_BUDGET_TOKEN = '<?php echo addslashes($budgetToken); ?>';
</script>

<!-- Script métier -->
<script src="/personnel/basi-scripts.bundle.11.js"></script>
