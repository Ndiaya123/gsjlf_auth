...
const AL_API = '/personnel/compta_basi_controller';
const al_api = {
...
function al_renderServices(services) {
    ...
    services.forEach(s => {
        ...
        o.value = s.id; o.textContent = s.nom_categorie +"/"+ s.nom_sous_categorie;
        sel.appendChild(o);
    });
}
...
async function al_filtrerService() {
    ...
    const categorieId = document.getElementById('al-sel-service')?.value || null;
    al_showLoader('Filtrage…');
    const [dBudget, dExist] = await Promise.all([
        al_post(al_api.lignesBudget, {
            budgetId: al_budgetToken,
            ...(categorieId ? { id_Sous_categorie: parseInt(categorieId) } : {}),
        }),
        al_post(al_api.lignesExist, { demandeId: parseInt(al_demande.id) }),
    ]);
    al_hideLoader();
