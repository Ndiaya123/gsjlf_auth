...
// ==================================================
// SYNC — contrôle fermeture loader (datatable + stats)
// ==================================================
...
function getYearInDakar() {
    const options = { timeZone: 'Africa/Dakar', year: 'numeric' };
        ...
        dt = $("#kt_datatable_utilisateurs").DataTable({
            ...
            ajax: {
                ...
                error  : function () {
                    ...
                    _checkCloseLoader();
                    Swal.fire('Erreur', "Impossible de charger la liste des élèves.", 'error');
                }
            },
            columns: [
                { data: "photo"        },
                ...
                },
                {
                    ...
                    render   : function (data, type, row) {
                        ...
                        if (etat2 === 1) {
                            ...
                            if (etat1 === 1) {
                                return '<a href="/personnel/ps1-responsable-financier-fichier-eleve/' + data + '" ' +
...
// ==================================================
// 4) READY — chargement filtres + bind onchange
// ==================================================
$(document).ready(function () {

    _resetLoaderSync();
    //showLoader("Chargement des filtres et de la liste des élèves…");
...
function setStatsLoading() {
    ["ps1-total", "ps1-actifs", "ps1-inactifs","ps1-bloquer",
        "ps1-pct-actifs", "ps1-pct-inactifs","ps1-pct-bloquer"].forEach(function (id) {
        setStatEl(id, "…");
    });
    setStatBar(0);
}
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            const fallback = function () {
                ["ps1-total", "ps1-actifs", "ps1-inactifs","ps1-bloquer",
                    "ps1-pct-actifs", "ps1-pct-inactifs","ps1-pct-bloquer"].forEach(function (id) { setStatEl(id, "—"); });
                setStatBar(0);
                _loaderSync.statsReady = true;
                _checkCloseLoader();
            };

            if (typeof data !== "string" || data.trim() === "" || !testJSON(data)) {
                fallback(); return;
            }
            ...
            const pctInactifs = total > 0 ? Math.round((inactifs / total) * 100) : 0;
            const pctBloquer = total > 0 ? Math.round((bloques / total) * 100) : 0;

            setStatEl("ps1-total",        total);
            setStatEl("ps1-actifs",       actifs);
            setStatEl("ps1-inactifs",     inactifs);
            setStatEl("ps1-bloquer",     inactifs);
            setStatEl("ps1-pct-actifs",   pctActifs   + "%");
            setStatEl("ps1-pct-inactifs", pctInactifs + "%");
            setStatEl("ps1-pct-bloquer", pctBloquer + "%");
        ...
        error: function () {
            ["ps1-total","ps1-actifs","ps1-inactifs",
                "ps1-pct-actifs","ps1-pct-inactifs"].forEach(function (id) { setStatEl(id, "—"); });
            setStatBar(0);
            _loaderSync.statsReady = true;
            _checkCloseLoader();
        }
